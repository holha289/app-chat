...
export default function ChatHeader({ name, avatar, onBack }: Props) {
  return (
    <View className="mt-10 flex-row items-center border-b border-gray-200 px-4 py-3">
      <TouchableOpacity className="h-10 w-10" onPress={onBack}>
        <Ionicons name="arrow-back-outline" size={24} color="#111827" />
      </TouchableOpacity>
      <Image source={{ uri: avatar || AVATAR(name) }} className="mr-3 h-10 w-10 rounded-full" />
